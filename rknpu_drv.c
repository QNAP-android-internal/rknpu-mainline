...
static struct drm_driver rknpu_drm_driver = {
	...
	/* 7.1: gem_prime_mmap removed (handled via DEFINE_DRM_GEM_FOPS .mmap). */
	.ioctls = rknpu_ioctls,
	.num_ioctls = ARRAY_SIZE(rknpu_ioctls),
	.fops = &rknpu_drm_driver_fops,
	.name = DRIVER_NAME,
	.desc = DRIVER_DESC,
	/* 7.1: drm_driver.date removed. */
	.major = DRIVER_MAJOR,
	.minor = DRIVER_MINOR,
...
static int rknpu_power_off(struct rknpu_device *rknpu_dev)
{
	...
	if (rknpu_dev->multiple_domains) {
		 ...
		 * it may run after the NPU power domain / clk / regulator are
		 * gated, and that callback touches NPU registers -> crash.
		 *
		 * Mainline port: the vendor rockchip_iommu_is_enabled() poll is
		 * unavailable, so mirror the FPGA path and give the IOMMU's
		 * asynchronous runtime-suspend a short window to settle before
		 * the power domains are cut.
		 */
